...
function js_input(dict){
	...
	*/}.toString().slice(15,-4);
	...
	$('*').attr('disabled','disabled');	//disable html functionality during prompt
	
	if(!dict.input_msg || dict.input_msg == ''){
		dict.input_msg='add input: ';
	}
	if(!dict.el || dict.el.length == 0){
	...
	//---------------position prompt in center of window------------
	...
	$(document).on('click','.js_input_button',function(){
		...
		$('#js_input_box').remove();		//remove input box
